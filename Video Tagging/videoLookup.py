...
def getImageTags(frame_keywords):
    ...
    image_search_title_texts = [
        i.text[: i.text.find("...")].strip() for i in links if "..." in i.text
    ]
    image_search_titles = []
    for i in image_search_title_texts:
        ...
        if k != -1:
            ...
            k = keywords.getKeywordsRAKE(i)
            image_search_titles.append(
                {"keywords": k, "entities": [], "categories": []}
            )
    ...
    print("Google rel searches:")
    print(google_relevant_searches)
...
def getTags(videofilename):
    ...
    path_to_images = objectDetection.clusterFrames(path_to_images)
    ...
    for count, p in enumerate(path_to_images[::4]):
        print("{}/{}".format(count + 1, l))
        tags.append(
            getTagsfromFrame(p, videofilename, keywords_translated, frame_objects)
        )
